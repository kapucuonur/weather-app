...
async function checkWeather(city) {
    ...
    const api_key = "c80e259dca249e73ac2138d8b0365025";
    const url = `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${api_key}`;

    try {
        // API çağrısına cache-busting ekleyerek verinin her defasında yenilenmesini sağlıyoruz
        const weather_data = await fetch(`${url}&_=${new Date().getTime()}`).then(response => response.json());
